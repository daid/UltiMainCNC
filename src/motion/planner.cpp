...
bool planner_buffer_line(const float (&position)[INPUT_AXIS_COUNT], float feed_rate)
{
    ...
    for(uint8_t n=0; n<OUTPUT_AXIS_COUNT; n++)
    {
        ...
        block->step_event_count = MAX(block->step_event_count, block->steps[n]);
        // Compute direction bits for this block
        if (target_step_position[n] < final_step_position[n])
            block->direction_bits |= _BV(n);
    }
    ...
    feed_rate = MAX(minimumfeedrate, feed_rate);
    ...
    if (moves_planned() > 1 && (previous_nominal_speed > 0.0001))
    {
        ...
        if(fabs(current_speed[2] - previous_speed[2]) > max_z_jerk)
            vmax_junction_factor = MIN(vmax_junction_factor, (max_z_jerk/fabs(current_speed[2] - previous_speed[2])));

...
typedef struct {
    ...
    unsigned long steps[OUTPUT_AXIS_COUNT];   // Step count along each axis
    unsigned long step_event_count;           // The number of step events required to complete this block
    unsigned long accelerate_until;           // The index of the step event on which to stop acceleration
    unsigned long decelerate_after;           // The index of the step event on which to start decelerating
    unsigned long acceleration_rate;          // The acceleration rate used for acceleration calculation
...
FORCE_INLINE void planner_discard_current_block()
{
...
FORCE_INLINE block_t *planner_get_current_block()
{
    if (block_buffer_head == block_buffer_tail)
        return NULL;
